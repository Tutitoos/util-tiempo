...
const getFormatDate = (options) => {
    const timestamp = Validate_1.default.timestamp(options?.timestamp);
    const local = Validate_1.default.locale(options?.local ?? "pt-PT");
    const timezone = Validate_1.default.timezone(options?.timezone);
    const hour12 = Validate_1.default.hour12(options?.hour12);
    ...
    for (const key of [
        ...
        for (const data of Object.values(key)) {
            let hour = Number(new Date(timestamp).toLocaleString(local, {
                timeZone: timezone,
                ...data,
            }));
            if (hour12) {
            ...
            const keyObject = Object.keys(key)[0];
            content = content.replace(key[keyObject].format, String(hour).padStart(key[keyObject].format.length - 2, "0"));
        }
    }
    for (const key of [
        ...
        for (const data of Object.values(key)) {
            const keyObject = Object.keys(key)[0];
            content = content.replace(key[keyObject].format, new Date(timestamp)
